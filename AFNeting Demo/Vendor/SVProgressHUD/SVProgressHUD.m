...
+ (SVProgressHUD*)sharedView {
    ...
    dispatch_once(&once, ^ {
		...
		sharedView.hudSize = CGSizeMake(100, 100);
	});
    return sharedView;
}
...
+ (void)setAttributeStatus:(NSMutableAttributedString *)string
{
    [[self sharedView] setAttributString:string];
}
...
+ (void)setHubHeight:(CGFloat)height
{
    [self sharedView];
    SVProgressHUDHeight = height;
}

+ (void)setHubWidth:(CGFloat)width
{
    [self sharedView];
...
- (void)updatePosition {
    ...
    NSLog(@".......fontsize......%f", self.stringLabel.font.pointSize);
    UIFont* newFont = [self.stringLabel.font fontWithSize:self.stringLabel.font.pointSize + 5];
    if(string) {
...
//        }
        ...
        CGFloat labelRectY = imageUsed ? 68 : hudHeight/2 - stringHeight/2;
        
        if(hudHeight > 100) {
            labelRect = CGRectMake(12, labelRectY, hudWidth - 24, stringHeight);
        } else {
...
- (void)setAttributString:(NSMutableAttributedString*)string
{
    ...
    [UIView animateWithDuration:0.15
                     ...
                     completion:^(BOOL finished){
                         if(self.alpha == 0 || self.hudView.alpha == 0) {
							 ...
							 self.hudSize = CGSizeMake(100, 100);
...
- (NSTimeInterval)displayDurationForString:(NSString*)string {
    return MIN((float)string.length*0.06 + 1.0, 5.0);
}
...
- (CAShapeLayer*)indefiniteAnimatedLayer {
    if(!_indefiniteAnimatedLayer) {
        ...
        UIBezierPath* smoothedPath = [UIBezierPath bezierPathWithArcCenter:arcCenter
                                                                    radius:self.radius
                                                                startAngle:M_PI*3.3/2  //M_PI*3/2
                                                                  endAngle:M_PI*3.3 //M_PI/2+M_PI*5
                                                                 clockwise:YES];
        ...
        bglayer.colors = @[(id)[UIColor redColor].CGColor,(id)[UIColor greenColor].CGColor,(id)[UIColor blueColor].CGColor];
        bglayer.startPoint = CGPointMake(0, 0);
        bglayer.endPoint = CGPointMake(0, 1.0);
        bglayer.frame = _indefiniteAnimatedLayer.bounds;
//        _indefiniteAnimatedLayer.mask = bglayer;
